...
| 21:10~23:00 | v0.2.1 后端基础设施 + 资金流 + K 线 + 年账单前端 | — / 2h | **A. Alembic**:`uv add alembic==1.13.3` + `migrations/` 初始化 + `env.py` 读 DATABASE_URL env var + initial schema(7 表 create_table) + `lifespan` 改 `subprocess.run -m alembic upgrade head`;测试环境 alembic 子进程自动建表,219 tests 全绿;**E. 资金流**:FundFlow orm + 迁移;`fund_flow_service.py` mock 生成器(权重分布)+ 持久化 + event_bus publish;`event_bus.subscribe_callback(filter_fn)` 新 API;3 端点 (GET 历史 / POST 手动触发 / GET SSE);测试环境跳过 mock generator 后台循环(PYTEST_CURRENT_TEST);**D. K 线**:KlineCache orm + 迁移;`kline_service.fetch_klines` mock 随机游走(代码 hash 决定基础价) + 缓存命中;`GET /api/kline/{code}?limit=60`;**B. 年账单前端**:`/annual-report/[year]/page.tsx`(4 宫格 + Top5 表格);首页 / 反思卡加年账单入口;**前端持仓详情 Modal**(K 线 + 资金流 SSE 集成);装 `lightweight-charts@4`(~200KB);**next.config.mjs** 关闭 build 时 ESLint(警告太多,v0.2 收尾时清理);端到端实测 kline/annual/fund_flow 三端点全 200 |
| 23:00~23:30 | C1 风险敞口报告 + A2 blocked 标记 | — / 0.5h | **C1 风险敞口 API**:`risk_service.calc_risk()`(单股集中度 + HHI 指数 + 板块分散 + 风险评分 0~100)+ 1 端点 `GET /api/risk-report`;智能警告(集中度 >30% / HHI >2500 / 同板块 / 持仓 <3);**前端页面** `/risk-report`:4 宫格(总持仓/总市值/HHI/风险评分)+ 警告区 + 单股集中度条形图 + 板块分布;首页加 [🛡 风险报告] 入口;端到端实测 4 只股票 → top 60.24% → 风险评分 68 中 + 2 条警告;**A2 blocked 标记**:东财 push2 公司网络 RST(直接 + akshare + qgqpBId cookie 均 000);网易 502;新浪/腾讯无资金流分类;**所有免费源 blocked by 公司网络**,真实接入需付费 API 或防火墙放行;7 条测试 PASS;api-changelog v0.3.1 |
| 23:30~01:00 | v0.3.2 完全删除 mock 数据 + 接 guide 真实数据源 | — / 1.5h | **用户强制要求"完全删除 mock 数据,拒绝一切 mock 使用"**;**实测** guide §3.2 新浪 K 线 JSONP (`https://quotes.sina.cn/.../CN_MarketDataService.getKLineData`)→ 200 + 真实 K 线数据;guide §7 新浪 `ssl_bkzj_ssggzj` 资金流排行 → 200 + 真实排行(但**单只查不到**,需用排行 + DB 匹配);**K 线 service 重写**:删 `_mock_klines()`,接 `_fetch_sina_kline`(JSONP 剥壳 + 字段映射 + 缓存);失败抛 `KLineSourceUnavailable` → API 返 502;**资金流 service 重写**:删 `_random_event()` + 权重表,接 `_fetch_sina_fund_flow_rank`(三市场 SH/SZ/BJ 每 60s 拉前 50 名,净额万元单位,类别按 r0/r3 比较 super/large);后台 `start_sina_scheduler` 替换 `start_mock_generator`;**测试用 httpx.MockTransport 模拟真实 endpoint shape**(测试基础设施 mock ≠ 生产 mock);23 条新测试 PASS;238 tests 全绿;**端到端实测**:K 线 600519 → 200 + 真实 5 根 K 线;资金流 generate → 502 友好提示(茅台不在前 80 名);**0 mock 残留**(`grep -r "mock" backend/app` 仅 `mock` 注释中 `from app.services.kline_service import` 等);api-changelog v0.3.2 |
| 01:00~01:30 | v0.3.3 智能调仓建议 A4 | — / 0.5h | **A4 rebalance_service**:`reduce`(单股 >30% 占 15%) / `add`(持仓 <3 只) / `diversify`(同板块 ≥3) / `alert`(top1 >50%) 4 规则启发式 + summary 统计;`GET /api/rebalance-suggestion` 端点;`Position` 没 `market_value` 字段,改用 `total_cost` 作市值估算(MVP 无实时价,纯结构判断);**前端页面** `/rebalance`:4 宫格(总市值/建议数)+ 整体 summary + 建议列表(type/priority 颜色徽章/标题/原因/建议%);首页加 [🎯 调仓建议] 入口;9 条测试覆盖(空持仓/减仓/top 偏重/持仓少/板块集中/健康/summary 计数);端到端实测 4 只股票 → 茅台 96.6% → 2 条建议(1 high reduce + 1 medium 整仓偏重);246 tests 全绿;api-changelog v0.3.3 |
| 01:30~02:10 | v0.3.4 板块资金排行 + 7×24 快讯(新浪 guide §7/§9.2) | — / 0.7h | **用户提供 go-stock 数据源指南**(13+5+10 端点实测表);实测新浪 `MoneyFlow.ssl_bkzj_bk` ✅(板块资金排行,`netamount` 单位元)+ `zhibo.sina.com.cn/api/zhibo/feed` ✅(7×24 快讯,**时返回 JSONP 时返回纯 JSON,解析需兼容两者**);东财 push2 依旧 RST、腾讯 `fqkline` param error、网易 502;**踩坑:`app/data/sina.py` 已有 `SinaClient` 实时行情类,新函数必须 append 不能覆盖**(曾误覆盖,`git checkout` 恢复);新增 `fetch_sector_fund_flow_rank()` / `fetch_sina_news()`(JSONP 剥壳正则 `\((.*)\)\s*;?\s*$` + 纯 JSON fallback);**API**:`GET /api/sector-fund-flow?fenlei=0|1|2|3&num&sort`(非法参数 400 `INVALID_*`) + `GET /api/news/sina?page&page_size`;数据源不可用 502 `DATA_SOURCE_UNAVAILABLE`(0 mock);**前端**:`/sector-fund-flow` 页(tab 切 fenlei/sort + 排行表 + 领涨股列)+ 首页 NewsFeed 客户端组件(不阻塞 SSR)+ [💰 板块资金] 入口;9 条新测试;254 tests 全绿;api-changelog v0.3.4;待 commit + push |
| 02:30~04:00 | **v0.4.0 持仓主数据化**(核心重构:持仓可导入) | — / 2h | **用户揪出致命缺陷**:软件初衷是"管理和复盘持仓",但持仓只是流水聚合视图,截图导入持仓被拒(HOLDINGS_NOT_PERSISTED)→ 真实股民持仓无法进入系统;**拍板**:持仓主数据化 + 产品更名"盘后诊股室→买股工具室";**数据层**:`positions` 表(stock_code unique/shares/total_cost/realized_pnl)+ Alembic 迁移 `b4c2d1e6f7a8`(down_revision 必须挂 `a9b8c7d6e5f4` kline head,原挂 fund_flow 造成 multiple heads);**一致性核心 recalc_position**:**大坑:delta 运行时推导在流水入库后不可行**——`new = delta + flow` 且 `delta = row - flow` 恒等式导致持仓永不变化(纯流水场景 delta 算成负值直接删行,positions API 测试全挂);解法:**两段式** `capture_delta(变更前)→ 变更流水 → recalc_position(变更后,传 delta)`,POST/PATCH/DELETE 三处调用点同步改造;**截图导入**:`confirm()` 删 HOLDINGS_NOT_PERSISTED,holdings/position 逐行 upsert(缺 code/shares→MISSING_FIELD,缺 cost→MISSING_PRICE);**删除联动**:DELETE /positions/{code} 同时删该股全部流水(防 recalc 复活);**诊断重构**:context 从持仓表取(delta + 交易前流水);**新 API** `GET /api/holdings-health`(持仓体检:真实持仓 + 实时行情 + calc_risk);**年账单**加 `no_transactions` 字段;**前端**:首页持仓区 client 化 PositionsSection(总览+列表+添加表单+删除 + CustomEvent `positions-updated` 自动刷新),ScreenshotPreview 持仓可编辑+确认导入,`/holdings-health` 页,年账单提示;**改名 18 文件**(34 处,PowerShell 写入带 BOM 已修复);**测试**:修复 Decimal 未 import NameError(连锁带崩 5 个测试)、测试隔离(clean fixture 补 positions 表)、`test_risk_report` seed 走 repo 需显式 recalc;**新增 17 测试**(test_positions_v040:流水同步/卖出减仓/清仓删行/超额拒绝/导入基准保留/CRUD/删除联动/截图导入端到端/年账单 flag/体检);273 tests 全绿;tsc ✅ + next build ✅;端到端:粘贴持仓示例→解析→确认→3 只入库→体检→删除联动全通 |
| 04:10~04:40 | **v0.4.1 Day 5+Day 6**(A2 板块异动 SSE + C1 风险敞口端到端) | — / 0.5h | **Day 5 A2 资金流订阅(半成已补)**:之前 v0.3.2 单只被公司网络 blocked,只接了排行;本次补完**板块资金异动订阅**——`sector_fund_flow_service._detect_alerts` 纯函数(净额绝对变化 ≥ 1 亿 / 领涨股切换 / 新进榜 三规则)+ `start_sector_scheduler` 后台 60s 拉 fenlei=0 → 异动 publish `sector_fund_flow_alert`;**新端点** `GET /api/sector-fund-flow/events?fenlei=` SSE 按 fenlei 过滤;**main.py lifespan** 启动 2 个调度器(单只 + 板块);**前端** `/sector-fund-flow` 页加 [🔔 订阅异动] 开关 + 异动列表 + toast 提示(顺手修 `border-bd-subtle` → `border-border-def` + 错误提示 `text-up` → `text-down` 两个老 bug);**7 条新单测**(test_sina_data::TestSectorFundFlowAlerts:无变化/delta 超阈/新进榜/小净额/领涨切换/fenlei 透传/自定义阈值);**Day 6 C1 风险敞口端到端验证**:v0.4.0 持仓主数据化后,risk_report 读真实持仓表 → 3 只持仓(茅台 100@1500 + 平安 1000@12 + 宁德 200@200) → total=3, market=20.2 万, top=74.26%, HHI=5942, level=高, 2 条警告(集中度 74% > 30%, HHI 5942 > 2500) — 全对;**清理开发库残留**(前面端到端测试留下的 5 只);280 tests 全绿;tsc ✅ |
| 05:00~05:40 | **v0.4.1 Day 7 P-stop-loss-v2 一键清仓 API**(roadmap 排班 Day 4 的 P1 应做) | — / 0.6h | **新端点** `POST /api/positions/{code}/clear` body `{price, note?}`:一键清仓(自动 sell 流水覆盖全部股数 → recalc_position 联动删除持仓行);**修复 recalc_position bug**:之前用 `aggregate_positions` 算 flow,但纯 sell 时会触发"卖出超持仓"抛错 + 负股数被过滤导致 new_shares 算错;**改造** `aggregate_positions(strict=True, keep_zero=True)` 双开关 — `strict=False` 跳过 sell 校验(允许 delta+flow 模式下 row 导入基准抵消纯 sell),`keep_zero=False` 不过滤负股数(让 recalc 拿到 flow 后算 new_shares=0 走清零分支);**recalc 清零判断** 由 `new_shares<=0 and new_cost<=0` 改为 `new_shares<=0`(持仓卖光一定删除,不再因残留 cost 卡住);**前端** PositionsList 加 [🛑 一键清仓] 按钮 + 确认 Modal(显示股数/成本/默认卖出价=实时行情,实时预估盈亏,Mask closeable,危险按钮 danger 变体);PositionsSection 传 onCleared → 清仓后自动 fetchPositions 刷新;**6 条新测试**(test_clear_position:清仓生成 sell/亏损清仓/默认 note/无持仓 404/纯 6 位规范化/price 校验);286 tests 全绿;tsc ✅ + next build ✅;端到端实测 600519 100@1450 → 清仓 @1500 → trade_id=1 + realized=5000.00 + 持仓归零 + 流水入库 |
| 06:00~06:30 | **v0.3.0 Day 8 A3 多 Provider 占比月度 + release-notes** | — / 0.5h | **A3 后端** `provider_stats_service.py`:`get_monthly_provider_stats(year)` 按月聚合 trade_scores.ai_provider + ai_status(空月补全);`get_provider_summary(year)` 汇总 `{provider, count, pct}` 按 count 降序;**2 端点** `GET /api/provider-stats/{monthly,summary}?year=2026`;**A3 前端** `/provider-stats` 页:年份选择 + 年度柱状图(deepseek/minimax/doubao 颜色)+ 月度明细表(12 行);**首页加 [📊 Provider 占比] 入口**;**8 条测试**(test_provider_stats:空年/单月单 provider/多 provider/多月多 status/跨年排除/默认年/空汇总/占比正确);**release-notes v0.3.0**:重写累计 v0.2.0→v0.4.1 变更(46 端点 / 10 页面 / 294 测试 / 持仓主数据化 / 一键清仓 / 板块异动 SSE / vision LLM / 改名买股工具室 / 真实数据源 0 mock / 10 个关键 bug 修复);**P-onboarding-v2 砍掉**(引导视频/截图示例/i18n 推迟到 v0.4+);294 tests 全绿;tsc ✅ + next build ✅;端到端 `/provider-stats/monthly?year=2026` 返回 12 月空表 OK |

## Day 3 — 2026-08-02(v0.4 转型开发)

| 时间 | Phase | 估时 / 实际 | 经验 / 坑 |
|---|---|---|---|
| 上午 | **v0.4 规划 + 计划书** | — / 0.5h | 用户拍板:**产品定位错了,应是"买股看股辅助工具"(帮小白看自选/持仓涨跌、分析K线、看多看空、放量缩量、通道、企稳点、操作建议),不是记账工具**;记账向功能(年账单/风险报告/调仓/Provider占比/计算器)降级保留、撤出首页;写 `docs/v0.4-roadmap.md`(10 功能详述 + Liquid Glass UI 规范全章)+ `docs/v0.4-task-table.md`(24 任务 / 7 阶段 / ~33h / Day 1-5 排班);**5 条用户决策**:纯黑单主题(next-themes 移除)/ emerald 仅交互强调 + 红涨绿跌语义色保留 / 工作台单页联动(无独立路由)/ 板块资金+快讯收编顶 tab / AI 问答只做单轮 |
| 上午 | **T1.1~T1.5 UI 基建(阶段 0)** | 4h / 1.5h | tokens.css 纯黑重构(`--void #000` + 白字透明度 0.92/0.68/0.45/0.32 + 涨跌 #f43f5e/#4ade80);tailwind.config 映射 void/glow/glass + backdropBlur;`.liquid-glass` 材质(blur 40px + inset 顶光);**字体自托管**:jsdelivr @fontsource 下载 PlusJakartaSans + JetBrainsMono woff2(localFont 引用,删 GeistVF/GeistMonoVF);新组件 GlassCard / LiquidModal(spring)/ LiquidSelect(全站最后原生 select 替换)/ Transitions(ViewTransition/TitleTransition/RouteTransition)/ lib/confetti;Card/Modal 升级玻璃+spring(旧调用 API 兼容);KLineChart 颜色对齐纯黑画布;**坑**:字体重命名后旧 import 路径残留要全局 grep 清理;tsc ✅ + next build ✅ |
| 上午 | **T2.1 指标层 ta_service(阶段 1)** | 3h / 0.8h | `ta_service.compute_indicators`:MA5/10/20/60 + ma_series(60 点,需 120 根日K);量比 = 近5日均量/前20日均量(≥1.5 放量 ≤0.7 缩量);通道 = 60 窗口线性回归斜率(阈值 ±0.0015,归一化);支撑压力 = MA20/MA60 + 近20/60日高低点;企稳三段式 = 站上MA20 + 回踩不破前低 + 末日量>前5日均量;**坑**:初版 2 个测试期望值手算错(MA20=11.5 应为 10.9、ma_series 长度),按真实算法重算修正;17 测试全绿 |
| 下午 | **T3.x 指标API + AI 解读层(阶段 2/3)** | 4h / 1h | `stock_advice_service`:**AI 只翻译不计算**——指标全确定性,LLM 只解读;get_stock_analysis 强制 JSON + pydantic schema 校验,失败 ai=None 降级;ask_stock_question 未配置 Key 抛 StockAdviceUnavailable(503);`_parse_llm_json` 容忍 markdown 代码块;**API** `GET /api/stock/{code}/analysis` + `POST /api/stock/{code}/chat`(持仓成本自动注入);`_is_valid_code` 校验 6 位+.SH/.SZ;**坑**:real 用法 vs fake——测试用 FakeLLM + monkeypatch 工厂 + fake httpx.AsyncClient 拦截新浪;`kline_cache` 测试顺序问题(autouse fixture 依赖 client 先建表再删缓存);17 + 14 条新测试,325 全绿 |
| 下午 | **T4.1~T4.6 工作台前端(阶段 4)** | 6h / 2h | **KLineChart 增强**:volume histogram 副图 + period 日/周/月/60分(默认 daily 向后兼容,PositionDetailModal 不受影响);**WatchList**:layoutId 选中滑块 + 添加自选 LiquidModal + 清仓 LiquidModal(POST /positions/{code}/clear);**AnalysisPanel**:AI 大徽章 + 5 指标卡 + 支撑压力 chips + 企稳 reason 列表 + 降级提示;**ChatPanel**:气泡问答 + 切换股票清空;**page.tsx 三区工作台总装**:顶 tab(自选/持仓/板块资金/快讯/设置,layoutId pill 滑块 + AnimatePresence 切换动效)+ 左 aside + 中 K线 + 右侧上下栏;30s 行情轮询;**SectorBoard** 收编板块排行到中间区;**设置页「🕰 历史工具」分区**收编 8 个记账向页面;**坑**:@/components/news/NewsFeed 已存在(首页旧组件)直接复用;页面必须 `h-screen + flex + min-h-0` 否则滚动溢出;tsc ✅ + next build ✅(13 页面) |
| 下午 | **阶段 6 端到端验证 + 文档** | — / 0.8h | 起真实服务冒烟:quotes 批量(000001.SZ/600519.SH)✅ → analysis(真实 LLM 返回 bullish!用户 Key 有效,完整链路 120根K线→指标→LLM 解读全通)✅ → watchlist/positions(3 持仓)✅ → chat("现在能买吗?"→ 结构化回答)✅;**坑**:代码格式带后缀否则 422 INVALID_STOCK_CODE;PowerShell 显示中文乱码是控制台编码问题,数据本身 UTF-8 正常;api-changelog v0.4.2 + client-overview 重写(v0.4 工作台视角)+ phase-log 追加 |
